        ...
        <table class="table table-striped">
            ...
            <tbody>
                @forelse($resultats as $resultat)
                    <tr>
                        <td>{{ $resultat->nom_banque }}</td>
                        <td>{{ $resultat->ville }}</td>
                        <td>{{ $resultat->groupe_sanguin }}</td>
                        <td>{{ $resultat->type_produit_sanguin }}</td>
                        <td>Nombre de poches de sang : {{ $resultat->nombre_poches }}</td>
                        <td><button id="pay-btn" class="btn btn-primary">commander</button></td>
  <script type="text/javascript">
      FedaPay.init('#pay-btn', { public_key: 'pk_live_ySlRyQtoOwk92_TSjmDJ2IKZ' });
  </script>
                        <form action="{{ route('commander') }}" method="POST">
  
        @csrf
        <input type="hidden" name="banque_id" value="{{ $resultat->banque_id }}">
        <input type="hidden" name="nombre_poche" value="{{ $resultat->nombre_poches }}">
    </form>
